...
class Game {
  constructor() {
    ...
    this.maxEnergy = 5;
    this.energy = 5;
    this.energyRegenTime = 2200; // ms per 1 energy
    this.lastEnergyRegen = Date.now();
    this.energyRegenProgress = 0; // 0-1
  ...
  drop() {
    ...
    // Energy check
    if (this.energy <= 0) {
      // Flash all pips red and show hint
      document.querySelectorAll('.energy-pip').forEach(p => {
        p.classList.remove('empty-flash');
        void p.offsetWidth;
        p.classList.add('empty-flash');
      });
      // Show "No Energy!" hint on canvas if not already shown
      const wrap = document.getElementById('canvas-wrap');
      if (!wrap.querySelector('.energy-empty-hint')) {
  ...
  updateEnergyUI() {
    ...
    pips.forEach((pip, i) => {
      ...
      const shouldFill = i < this.energy;
      if (shouldFill && !wasFilled) {
        pip.classList.add('filled', 'flash');
        setTimeout(() => pip.classList.remove('flash'), 300);
      } else if (!shouldFill) {
        pip.classList.remove('filled', 'flash');
      }
    });
    // Show/hide "no energy" cursor on canvas
    const wrap = document.getElementById('canvas-wrap');
    if (this.energy <= 0) wrap.classList.add('no-energy');
    else wrap.classList.remove('no-energy');
  }
  ...
  updateEnergyRegen() {
    ...
    if (this.energy >= this.maxEnergy) {
      this.energyRegenProgress = 0;
      document.getElementById('energy-regen-bar').style.width = '0%';
      return;
    }
    const now = Date.now();
    const elapsed = now - this.lastEnergyRegen;
    this.energyRegenProgress = Math.min(1, elapsed / this.energyRegenTime);
    document.getElementById('energy-regen-bar').style.width = (this.energyRegenProgress * 100) + '%';

    if (elapsed >= this.energyRegenTime) {
      this.energy = Math.min(this.maxEnergy, this.energy + 1);
      this.lastEnergyRegen = now;
      this.energyRegenProgress = 0;
      this.updateEnergyUI();
      // Play a soft regen sound
      this.playRegenChime();
    }
  }
