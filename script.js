...
class FruitFusionGame {
    constructor() {
        ...
        // Physics like sample-1 (Matter.js equivalent) - mobile more calm
        this.gravity = this.isMobile ? 0.4 : 0.5;
        this.friction = this.isMobile ? 0.6 : 0.4; // Higher friction on mobile
        this.restitution = this.isMobile ? 0.1 : 0.3; // Less bounce on mobile
        this.airResistance = this.isMobile ? 0.98 : 0.999; // More air resistance on mobile
    ...
    handleMouseMove(e) {
        ...
        // Only update drop position on PC (not mobile)
        if (!this.isMobile) {
    ...
    handleDrop(e) {
        ...
        // On mobile, drop at click position. On PC, use current dropX
        let dropX = this.dropX;
        if (this.isMobile && e.clientX !== undefined) {
            const rect = this.canvas.getBoundingClientRect();
            const clickX = (e.clientX - rect.left) * (this.canvas.width / rect.width);
            const fruitSize = this.fruitTypes[this.nextFruitType].size;
            dropX = Math.max(fruitSize, Math.min(this.canvas.width - fruitSize, clickX));
        }

        // Create new fruit at drop position
        ...
        const fruit = this.createFruit(dropX, dropY, fruitType);
    ...
    createBubble(x, y, radius) {
        return {
            x: x,
            y: y,
            radius: radius + 4, // Slightly larger for solid collision
            opacity: 0.25,
    ...
    updatePhysics() {
        for (let i = 0; i < this.fruits.length; i++) {
            ...
            // Additional damping on mobile for calmer movement
            if (this.isMobile) {
                fruit.vx *= 0.98;
                fruit.vy *= 0.98;
            }

            // Stop very small movements - fruits must come to rest
            const minVelocity = 0.05;
            if (Math.abs(fruit.vx) < minVelocity) fruit.vx = 0;
            if (Math.abs(fruit.vy) < minVelocity) fruit.vy = 0;

            // Progressive friction - slower fruits get more friction
            const speed = Math.sqrt(fruit.vx * fruit.vx + fruit.vy * fruit.vy);
            if (speed < 1.0) {
                const progressiveFriction = 0.95 - (speed * 0.1); // More friction for slower fruits
                fruit.vx *= progressiveFriction;
                fruit.vy *= progressiveFriction;
            }

            // STABILITY CHECK - prevent erratic movement from pressure
            const isOnFloor = (fruit.y + fruit.bubble.radius >= this.floorY - 1);
            if (isOnFloor) {
                // Fruits on floor should be more stable
                fruit.vx *= 0.9; // Extra friction on floor
            ...
            // SOLID FLOOR ENFORCEMENT - prevent sinking
            if (fruit.y + fruit.bubble.radius > this.floorY) {
                fruit.y = this.floorY - fruit.bubble.radius;
                fruit.vy = 0;

                // Stop any jittery movement on floor
    ...
    handleWallCollisions(fruit) {
        // Left wall - solid boundary
        if (fruit.x - fruit.bubble.radius < this.wallThickness) {
            fruit.x = this.wallThickness + fruit.bubble.radius;
            fruit.vx = -fruit.vx * this.restitution;
        }

        // Right wall - solid boundary
        if (fruit.x + fruit.bubble.radius > this.canvas.width - this.wallThickness) {
            fruit.x = this.canvas.width - this.wallThickness - fruit.bubble.radius;
            fruit.vx = -fruit.vx * this.restitution;
        }

        // Floor - SOLID FLOOR using bubble radius
        if (fruit.y + fruit.bubble.radius > this.floorY) {
            fruit.y = this.floorY - fruit.bubble.radius; // Solid positioning
            fruit.vy = 0; // Stop vertical movement completely
    ...
    handleFruitCollision(fruit1, fruit2) {
        ...
        // SMART COLLISION - different behavior for same vs different fruits
        const sameType = fruit1.type === fruit2.type && fruit1.emoji === fruit2.emoji;

        let minDistance;
        if (sameType) {
            // Same fruits - allow closer contact for merging
            minDistance = fruit1.radius + fruit2.radius + 1;
        } else {
            // Different fruits - solid bubble collision
            minDistance = fruit1.bubble.radius + fruit2.bubble.radius;
        }

        if (distance < minDistance && distance > 0) {
            // SOLID SEPARATION - prevent sinking
            const overlap = minDistance - distance;
            const separationX = (dx / distance) * overlap * 0.5;
            const separationY = (dy / distance) * overlap * 0.5;

            // Apply separation but respect floor boundaries
            fruit1.x -= separationX;
            fruit1.y -= separationY;
            fruit2.x += separationX;
            fruit2.y += separationY;

            // Ensure fruits don't sink below floor
            if (fruit1.y + fruit1.bubble.radius > this.floorY) {
                fruit1.y = this.floorY - fruit1.bubble.radius;
            }
            if (fruit2.y + fruit2.bubble.radius > this.floorY) {
                fruit2.y = this.floorY - fruit2.bubble.radius;
            }
            ...
            // Different collision response for same vs different fruits
            let restitution, dampingFactor;
            if (sameType) {
                // Same fruits - gentle collision for merging
                restitution = this.isMobile ? 0.05 : 0.1;
                dampingFactor = this.isMobile ? 0.5 : 0.6;
            } else {
                // Different fruits - normal collision
                restitution = this.isMobile ? this.restitution * 0.5 : this.restitution;
                dampingFactor = this.isMobile ? 0.7 : 0.8;
            }
            ...
            fruit1.vx -= impulse1 * normalX * dampingFactor;
            fruit1.vy -= impulse1 * normalY * dampingFactor;
            fruit2.vx += impulse2 * normalX * dampingFactor;
            fruit2.vy += impulse2 * normalY * dampingFactor;

            // Rolling effect only for different fruits
            if (!sameType) {
                const tangentX = -normalY;
                const tangentY = normalX;
                const tangentialVelocity = relativeVelocityX * tangentX + relativeVelocityY * tangentY;
                const rollingForce = tangentialVelocity * 0.1;

                fruit1.vx += rollingForce * tangentX;
                fruit1.vy += rollingForce * tangentY;
                fruit2.vx -= rollingForce * tangentX;
                fruit2.vy -= rollingForce * tangentY;
            }
        }
    }
        ...
        for (let i = 0; i < fruitsSnapshot.length; i++) {
            for (let j = i + 1; j < fruitsSnapshot.length; j++) {
                ...
                if (liveFruit1.type === liveFruit2.type &&
                    ...
                    // MERGE DETECTION - use bubble radius for same type fruits
                    const mergeDistance = liveFruit1.bubble.radius + liveFruit2.bubble.radius - 2;
